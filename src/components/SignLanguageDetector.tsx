    ...
    const fingerStates = fingertips.map((tip, i) => {
      ...
      if (i === 0) {
        const distance = Math.sqrt(
          Math.pow(tip[0] - palmBase[0], 2) + 
          Math.pow(tip[1] - palmBase[1], 2)
        );
        return distance > 0.1 ? 1 : 0;
      } else {
        // For other fingers, check if they're extended (y-coordinate is above the knuckle)
        return tip[1] < knuckles[i][1] ? 1 : 0;
      }
    });
